		...
		<section className="py-20 bg-surface-alt px-4 md:px-8 transition-colors duration-300">
				...
				<div className="grid md:grid-cols-3  gap-8">
					{services.map((service, idx) => (
						<motion.div
							...
							className="p-8 bg-card rounded-lg border border-border hover:shadow-lg transition-shadow cursor-pointer"
						>
							{/* Service Image */}
							<div className="mb-4 rounded-lg overflow-hidden h-48 bg-surface-alt">
								<img
									src={service.icon}
									alt={service.title}
			...
			<AnimatePresence>
				{selectedService && (
					...
					>
						<motion.div
							className="bg-card rounded-xl p-6 md:p-8 max-w-2xl w-full shadow-xl relative max-h-[90vh] overflow-y-auto border border-border"
						...
						>
							<button
								className="absolute top-4 right-4 text-muted hover:text-heading z-10 bg-card rounded-full p-1"
								onClick={() => setSelectedService(null)}
							>
								<X size={22} />
							</button>
							...
							{/* Modal Image */}
							<div className="mb-6 rounded-lg overflow-hidden h-64 bg-surface-alt">
								<img
							...
							<ul className="space-y-2">
								{selectedService.details.projects.map(
									(proj, i) => (
										<li
											key={i}
											className="flex flex-col bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-lg p-3 hover:bg-green-100 dark:hover:bg-green-900/30 transition"
										>
											<a
												href={proj.link}
												target="_blank"
